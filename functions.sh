...
# like so:
# wget -q https://github.com/EasyMile/AppImages/raw/{chset}/functions.sh -O ./functions.sh
# . ./functions.sh

# RECIPE=$(realpath "$0")

# could be a branch, changeset or tag.
APP_IMAGE_CHSET=master
...
get_apprun()
{
  # wget -c https://github.com/EasyMile/AppImageKit/releases/download/5/AppRun -O ./AppRun # 64-bit
  wget -c https://github.com/EasyMile/AppImageKit/releases/download/continuous/AppRun-x86_64 -O AppRun # 64-bit
  chmod a+x AppRun
}
...
delete_blacklisted()
{
    BLACKLISTED_FILES=$(cat_file_from_url https://github.com/EasyMile/AppImages/raw/$APP_IMAGE_CHSET/excludelist | sed 's|#.*||g')
...
get_desktopintegration()
{
  ...
  cat_file_from_url https://raw.githubusercontent.com/EasyMile/AppImageKit/$APP_IMAGE_CHSET/desktopintegration > ./usr/bin/$REALBIN.wrapper
...
generate_type2_appimage()
{
  ...
  if ( [ ! -z "$KEY" ] ) && ( ! -z "$TRAVIS" ) ; then
    wget https://github.com/EasyMile/AppImageKit/files/584665/data.zip -O data.tar.gz.gpg
...
generate_status()
{
  ...
  wget -q -c "https://github.com/EasyMile/AppImages/raw/$APP_IMAGE_CHSET/excludedeblist"
